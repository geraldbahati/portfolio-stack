...
describe("cache", () => {
  it("marks auth and admin surfaces private", () => {
    ...
    expect(cacheControlForPath("/projects")).toContain("s-maxage=60");
    expect(cacheControlForPath("/projects/webline-store")).toContain("stale-while-revalidate=300");
    expect(cacheControlForPath("/sitemap.xml")).toContain("s-maxage=60");
